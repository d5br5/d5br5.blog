...
Next.js 를 사용하는 회사도 점점 많아지고 있는 것 같습니다.
...
tailwind css는 1번 단계에서 Next.js와 함께 설치 완료되었다.
Headless component library인 shadcn/ui를 설치해보자.

[shadcn/ui 공식문서](https://ui.shadcn.com/docs/installation/next)를 참고하자.
...
`components.json` 파일이 생성됨과 동시에, 설치가 완료된다.

### 3. 디렉토리 구조 설계

프로젝트 내부 폴더 구조는 다음과 같이 설계하였다.
...
위 코드에서 노란 음영 처리된 부분을 [Gray-matter](https://github.com/jonschlinkert/gray-matter)가 읽고 Metadata로 parsing 한다.
(아래에서 자세하게 설명할 예정)
